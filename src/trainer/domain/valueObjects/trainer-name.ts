...
export class TrainerName extends ValueObject<TrainerName> {
    ...
    private constructor(name: string) {
        ...
        if (!name ) throw new EmptyTrainerNameException( "El nombre del trainer esta vacio" );
        if (!patron.test(name)) throw new InvalidTrainerNameException( `El nombre del trainer ${name} no puede 
